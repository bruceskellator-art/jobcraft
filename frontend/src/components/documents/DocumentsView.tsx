...
function SimpleMarkdown({ content }: { content: string }) {
  ...
  return (
    <div className="text-sm text-foreground space-y-1 leading-relaxed">
      {lines.map((line, i) => {
        if (line.startsWith('## ')) return <div key={i} className="font-semibold text-foreground mt-2">{line.slice(3)}</div>
        if (line.startsWith('# ')) return <div key={i} className="font-bold text-foreground text-base mt-3">{line.slice(2)}</div>
...
function PreviewModal({
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-border flex-none">
          <div>
            <h2 className="text-sm font-semibold">
              {artifact.kind === 'resume' ? 'Résumé preview' : 'Cover letter preview'}
            </h2>
            {artifact.template_id && (
              <p className="text-xs text-muted-foreground mt-0.5">Template: {artifact.template_id}</p>
            )}
          </div>
        ...
        <div className="overflow-y-auto p-6 flex-1">
          ...
          {!isLoading && canPreviewHtml && !html && (
            <div className="text-sm text-muted-foreground text-center py-8">Preview unavailable.</div>
          )}
        </div>
      </div>
    </div>
          ...
          <>
            ...
            <section className="bg-card border border-border rounded-xl">
              <div className="px-4 py-3 border-b border-border flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg border border-border grid place-items-center bg-muted flex-none">
                  <svg
